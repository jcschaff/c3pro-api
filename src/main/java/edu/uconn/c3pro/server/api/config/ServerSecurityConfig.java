...
import edu.uconn.c3pro.server.api.security.JWTAuthorizationFilter;
...
public class ServerSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
        		.antMatchers("/apihealth").permitAll()			 // not to be exposed to public api (used for internal health monitoring)
        		.antMatchers("/c3pro/fhirenc/Contract").authenticated()
        		.antMatchers("/c3pro/fhirenc/QuestionnaireResponse").authenticated()
        		.antMatchers("/c3pro/fhirenc/Observation").authenticated()
        		.antMatchers("/c3pro/fhir/Questionnaire/{id}").authenticated()
        		.anyRequest().denyAll()
            .and()
